...
import { composeSlide, OUTPUT_SIZES, type ResolvedConfig } from "@appforge/screenshot-gen";
...
export async function POST(req: NextRequest) {
  ...
  try { await access(screenshotsDir); } catch { return NextResponse.json({ error: "SESSION_NOT_FOUND" }, { status: 404 }); }

  const targetSize = OUTPUT_SIZES.find((s) => s.suffix === outputSize);
  if (!targetSize) return NextResponse.json({ error: "INVALID_SIZE" }, { status: 400 });
  ...
  const config: ResolvedConfig = {
    ...
    outputDir: screenshotsDir, screenshotsDir,
    outputSizes: [targetSize], slides: [slide],
  };

  try {
    const fullBuffer = await composeSlide(slide, config, targetSize);
    const previewBuffer = await sharp(fullBuffer)
      .resize({ width: Math.round(targetSize.width / 2) }).png().toBuffer();
